...
1.  **Validación del `Origin`:** Nunca se debe reflejar el valor de la cabecera `Origin` sin validarlo contra una lista blanca de dominios permitidos.
2.  **Credenciales y CORS:** Si `Access-Control-Allow-Credentials: true`, el riesgo es máximo, ya que un atacante puede hacer peticiones autenticadas desde un dominio malicioso.
3.  **Impacto:** Una mala configuración CORS puede permitir el robo de datos privados de usuarios, tokens de sesión y otra información sensible.
